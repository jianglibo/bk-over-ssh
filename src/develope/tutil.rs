use crate::data_shape::{demo_app_conf, AppConf, AppRole, Server};
...
impl TestDir {
    ...
    pub fn create_sub_dir(&self, dir_name: impl AsRef<str>) -> PathBuf {
        let b = self.tmp_dir_path().join(dir_name.as_ref());
        fs::create_dir_all(b.as_path()).expect("tutil should create sub directory.");
        b.canonicalize().expect("tutil should canonicalize.")
    }
    ...
    pub fn make_a_file_with_len(
        ...
        make_a_file_with_len(self.tmp_dir.path(), file_name, file_len)
    }
}
