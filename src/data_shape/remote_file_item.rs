        ...
        match metadata_r {
            Ok(metadata) => {
                ...
                if let Some(relative) = relative_o {
                    ...
                    error!("RemoteFileItemOwned path name to_str() failed. {:?}", path);
                }
                // }
            }
            Err(err) => {
                error!("RemoteFileItemOwned from_path failed: {:?}, {:?}", path, err);
            }
        }
        None
    }
...
// impl<'a> std::convert::From<&'a RemoteFileItemOwnedOwned> for RemoteFileItemOwned<'a> {
//     fn from(rfio: &'a RemoteFileItemOwnedOwned) -> Self {
//         Self {
...
// #[derive(Debug, Deserialize, Serialize)]
// pub struct RemoteFileItemOwned<'a> {
//     path: &'a str,
//     sha1: Option<&'a str>,
//     len: u64,
//     created: Option<u64>,
...
mod tests {
    ...
    fn t_deserialize_item() -> Result<(), failure::Error> {
        ...
        // let item: RemoteFileItemOwned = RemoteFileItemOwned::from(&item_owned);
        ...
        let s = r##"{"path":"b b/b b.txt","sha1":null,"len":5,"created":1565607566,"modified":1565607566}"##;
        let fi = serde_json::from_str::<RemoteFileItemOwned>(s)?;
